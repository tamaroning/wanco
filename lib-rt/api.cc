...
extern "C" void
print (ExecEnv *exec_env, int32_t offset, int32_t len)
{
  for (int i = 0; i < len; i++)
    {
...
extern "C" void
print_i32 (ExecEnv *exec_env, int32_t i32)
{
...
extern "C" void
set_pc_to_frame (ExecEnv *exec_env, int32_t fn_index, int32_t pc)
{
  ASSERT (exec_env->migration_state
	    == wanco::MigrationState::STATE_CHECKPOINT_CONTINUE
	  && "Invalid migration state");
...
extern "C" void
push_local_i32 (ExecEnv *exec_env, int32_t i32)
{
  ASSERT (exec_env->migration_state
	    == wanco::MigrationState::STATE_CHECKPOINT_CONTINUE
	  && "Invalid migration state");
...
extern "C" void
push_local_i64 (ExecEnv *exec_env, int64_t i64)
{
  ASSERT (exec_env->migration_state
	    == wanco::MigrationState::STATE_CHECKPOINT_CONTINUE
	  && "Invalid migration state");
...
extern "C" void
push_local_f32 (ExecEnv *exec_env, float f32)
{
  ASSERT (exec_env->migration_state
	    == wanco::MigrationState::STATE_CHECKPOINT_CONTINUE
	  && "Invalid migration state");
...
extern "C" void
push_local_f64 (ExecEnv *exec_env, double f64)
{
  ASSERT (exec_env->migration_state
	    == wanco::MigrationState::STATE_CHECKPOINT_CONTINUE
	  && "Invalid migration state");
...
// stack
extern "C" void
push_i32 (ExecEnv *exec_env, int32_t i32)
{
  ASSERT (exec_env->migration_state
	    == wanco::MigrationState::STATE_CHECKPOINT_CONTINUE
	  && "Invalid migration state");
...
extern "C" void
push_i64 (ExecEnv *exec_env, int64_t i64)
{
  ASSERT (exec_env->migration_state
	    == wanco::MigrationState::STATE_CHECKPOINT_CONTINUE
	  && "Invalid migration state");
...
extern "C" void
push_f32 (ExecEnv *exec_env, float f32)
{
  ASSERT (exec_env->migration_state
	    == wanco::MigrationState::STATE_CHECKPOINT_CONTINUE
	  && "Invalid migration state");
...
extern "C" void
push_f64 (ExecEnv *exec_env, double f64)
{
  ASSERT (exec_env->migration_state
	    == wanco::MigrationState::STATE_CHECKPOINT_CONTINUE
	  && "Invalid migration state");
...
// globals
extern "C" void
push_global_i32 (ExecEnv *exec_env, int32_t i32)
{
  ASSERT (exec_env->migration_state
	    == wanco::MigrationState::STATE_CHECKPOINT_CONTINUE
	  && "Invalid migration state");
...
extern "C" void
push_global_i64 (ExecEnv *exec_env, int64_t i64)
{
  ASSERT (exec_env->migration_state
	    == wanco::MigrationState::STATE_CHECKPOINT_CONTINUE
	  && "Invalid migration state");
...
extern "C" void
push_global_f32 (ExecEnv *exec_env, float f32)
{
  ASSERT (exec_env->migration_state
	    == wanco::MigrationState::STATE_CHECKPOINT_CONTINUE
	  && "Invalid migration state");
...
extern "C" void
push_global_f64 (ExecEnv *exec_env, double f64)
{
  ASSERT (exec_env->migration_state
	    == wanco::MigrationState::STATE_CHECKPOINT_CONTINUE
	  && "Invalid migration state");
...
extern "C" int32_t
get_pc_from_frame (ExecEnv *exec_env)
{
  ASSERT (!wanco::chkpt.frames.empty () && "No frame to restore");
  ASSERT (exec_env->migration_state == wanco::MigrationState::STATE_RESTORE
	  && "Invalid migration state");
  int32_t ret = wanco::chkpt.frames.front ().pc;
  std::cerr << "[debug] call to get_pc_from_frame -> " << ret << std::endl;
  return ret;
}

extern "C" int32_t
pop_front_local_i32 (ExecEnv *exec_env)
{
  ASSERT (!wanco::chkpt.frames.empty () && "No frame to restore");
...
extern "C" int64_t
pop_front_local_i64 (ExecEnv *exec_env)
{
  ASSERT (!wanco::chkpt.frames.empty () && "No frame to restore");
  ASSERT (!wanco::chkpt.frames.front ().locals.empty () && "No local to pop");
  wanco::Value v = wanco::chkpt.frames.front ().locals.front ();
...
extern "C" float
pop_front_local_f32 (ExecEnv *exec_env)
{
  ASSERT (!wanco::chkpt.frames.empty () && "No frame to restore");
  ASSERT (!wanco::chkpt.frames.front ().locals.empty () && "No local to pop");
  wanco::Value v = wanco::chkpt.frames.front ().locals.front ();
...
extern "C" double
pop_front_local_f64 (ExecEnv *exec_env)
{
  ASSERT (!wanco::chkpt.frames.empty () && "No frame to restore");
  ASSERT (!wanco::chkpt.frames.front ().locals.empty () && "No local to pop");
  wanco::Value v = wanco::chkpt.frames.front ().locals.front ();
...
{
  std::cerr << "[info] Intercepted" << std::endl;
  ASSERT (exec_env->migration_state
