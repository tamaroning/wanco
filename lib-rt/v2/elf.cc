...
static int
find_elf_section (struct dl_phdr_info *info, size_t size, void *data);
      ...
      if (phdr.p_type == PT_LOAD)
	{
	  ...
	  if (phdr.p_flags & PF_X)
		...
		= reinterpret_cast<const ElfW (Ehdr) *> (segment_start);
	      const ElfW (Shdr) *shdr = reinterpret_cast<const ElfW (Shdr) *> (
		segment_start + ehdr->e_shoff);
	      ...
	      for (int j = 0; j < ehdr->e_shnum; ++j)
		{
		  ...
		  if (strcmp (sectionName, targetSection->name) == 0)
			...
			= segment_start + shdr[j].sh_offset;
